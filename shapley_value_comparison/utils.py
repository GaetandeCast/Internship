...
def led_mini(LMDI, SHAP, SAAB, X, idx, save_figure="saved_figures/"):
    ...
    map_object = LinearSegmentedColormap.from_list(
        name="red_transparent_blue", colors=color_array
    )
...
def digits_mini(LMDI, SHAP, SAAB, X, idx, save_figure="saved_figures/"):
    ...
    ff = axs[3].imshow(
        imp_SHAP.reshape(8, 8), cmap="red_transparent_blue", vmin=-A, vmax=A, extent=EE
    )
